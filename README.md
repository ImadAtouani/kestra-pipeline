# Kestra Pipeline - Orchestration de Normalisation de Données

## 📋 Description

Ce projet implémente un pipeline complet de normalisation de données orchestré par **Kestra**, avec :
...
- **8GB RAM** minimum
- **Ports disponibles** : 27018, 3000, 9090, 3200, 4317, 4319, 55680, 8890, 8080

### 1. Créer et préparer le projet

```bash
# Créer le dossier du projet
mkdir kestra-pipeline
cd kestra-pipeline
...
docker build -t kestra-pipeline:latest .
```
...
```
NAME                                STATE    PORTS
kestra-kestra-1                     Up       0.0.0.0:8080->8080/tcp, 0.0.0.0:8081->8081/tcp
kestra-postgres-1                   Up (healthy)   5432/tcp
kestra-mongodb-1                    Up (healthy)   0.0.0.0:27018->27017/tcp
kestra-grafana-1                    Up       0.0.0.0:3000->3000/tcp
kestra-otel_collector-1             Up       0.0.0.0:4317->4317/tcp, 0.0.0.0:4319->4318/tcp, 0.0.0.0:8890->8890/tcp
kestra-prometheus-1                 Up       0.0.0.0:9090->9090/tcp
kestra-tempo-1                      Up       0.0.0.0:3200->3200/tcp, 0.0.0.0:55680->55680/tcp
```
...
| **MongoDB** | mongodb://admin:changeme@localhost:27018 | admin / changeme |
...
> - **OTEL Collector Metrics** peut être vide au premier lancement. Les métriques apparaîtront après la première exécution du Flow Kestra.

### Premier accès à Kestra

1. Ouvrez http://localhost:8080 dans votre navigateur
2. Connectez-vous avec les identifiants par défaut : `admin@kestra.io` / `Admin1234!`
...
### Méthode 1 : Via l'interface Kestra (Recommandé)

#### Étape 1 : Importer le Flow

1. Connectez-vous à Kestra (http://localhost:8080)
2. Dans le menu, allez dans **"Flows"** → **"Create Flow"**
3. Copiez le contenu du fichier `flow.yml`
4. Cliquez sur **"Save"** pour enregistrer le flow

#### Étape 2 : Exécuter le Flow avec différents inputs
...
| **SQL** | `{"source_type":"sql","source_path":"postgresql://kestra:k3str4@postgres:5432/kestra","query":"SELECT * FROM information_schema.tables LIMIT 5"}` |

#### Étape 3 : Vérifier les résultats

```bash
# Connexion à MongoDB
docker exec -it kestra-mongodb-1 mongosh -u admin -p changeme
...
# Résultat attendu :
# pipeline_raw_data_total_ratio 5
# pipeline_normalized_data_total_ratio 5
# pipeline_rejected_data_total_ratio 0
# pipeline_raw_pending_total_ratio 0
# pipeline_raw_processing_total_ratio 0
# pipeline_raw_by_source_ratio{source_type="api"} 1
# pipeline_raw_by_source_ratio{source_type="csv"} 1
# pipeline_raw_by_source_ratio{source_type="excel"} 1
# pipeline_raw_by_source_ratio{source_type="html"} 1
# pipeline_raw_by_source_ratio{source_type="json"} 1
# pipeline_latency_last_milliseconds{script="ingestion"} 13.92
# pipeline_cpu_percent{script="ingestion"} 6.64
# pipeline_memory_mb_MB{script="ingestion"} 28.86
# pipeline_error_rate_percent{script="ingestion"} 0
```
...
docker exec -it kestra-mongodb-1 mongosh -u admin -p changeme --eval '
use data_pipeline;
...
db.raw_data.insertMany([
    ...
    {
        source_type: "json",
        source_path: "/app/data/products.json",
        ingested_at: new Date(),
        step: "ingestion",
        raw_payload: {
            data: {
...
|----------|-------------|-------|
| `pipeline_raw_data_total_ratio` | Nombre total de données brutes | 1 |
| `pipeline_normalized_data_total_ratio` | Nombre total de données normalisées | 1 |
| `pipeline_rejected_data_total_ratio` | Nombre total de données rejetées | 1 |
| `pipeline_raw_by_source_ratio` | Répartition des données par source | 1 |
| `pipeline_latency_last_milliseconds` | Dernière durée d'exécution par script | ms |
| `pipeline_latency_avg_milliseconds` | Durée moyenne d'exécution par script | ms |
| `pipeline_cpu_percent` | CPU par script | % |
| `pipeline_cpu_avg_percent` | CPU moyenne par script | % |
| `pipeline_memory_mb_MB` | Mémoire par script | MB |
| `pipeline_memory_avg_MB` | Mémoire moyenne par script | MB |
| `pipeline_errors_total_ratio` | Nombre d'erreurs par script | 1 |
| `pipeline_success_total_ratio` | Nombre de succès par script | 1 |
| `pipeline_error_rate_percent` | Taux d'erreur par script | % |
...
```bash
# Données brutes
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_raw_data_total_ratio'

# Données normalisées
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_normalized_data_total_ratio'

# Données rejetées
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_rejected_data_total_ratio'

# Latence
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_latency_last_milliseconds'

# CPU
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_cpu_percent'

# Mémoire
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_memory_mb_MB'

# Taux d'erreur
curl -s 'http://localhost:9090/api/v1/query?query=pipeline_error_rate_percent'
```
...
docker cp data/. kestra-kestra-1:/app/data/

# Vérifier
docker exec -it kestra-kestra-1 ls -la /app/data/
```
...
netstat -ano | findstr "4317 4318 4319 8890 9090 3000 3200 55680 8080 27018"
...
curl -X POST -g 'http://localhost:9090/api/v1/admin/tsdb/delete_series?match[]={__name__=~"pipeline.*"}'
...
|---------|--------------|--------------|-------|
| Kestra | 8080 | 8080 | Interface Web |
| Kestra API | 8081 | 8081 | API Kestra |
| MongoDB | 27018 | 27017 | Base de données |
...
|---------|------|-------------|
| v1.0.0 | 2026-06-30 | Version finale |
...
**Prêt à utiliser !** 🚀 Lancez `docker-compose up -d` et commencez à orchestrer vos données avec Kestra.